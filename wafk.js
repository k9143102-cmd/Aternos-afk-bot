...
// 4. http (Built-in Node module - ADDED FOR UPTIME FIX)
...
const http = require('http'); // <-- NEW: Required to keep the process alive on Render/Replit

// Load environment variables from config.env file
dotenv.config({ path: 'config.env' })

// --- UPTIME FIX: DUMMY WEB SERVER SETUP ---
// This server binds the required port (e.g., 3000) so the hosting service
// (Render/Replit) sees a running process and does not shut it down.
const uptimePort = process.env.PORT || 3000;

http.createServer((req, res) => {
    // Basic response for health checks
    res.writeHead(200, { 'Content-Type': 'text/plain' });
    res.end('Aternos Bot Cluster is Active.');
}).listen(uptimePort, () => {
    console.log(`\n[UPTIME FIX] Web Server bound to port ${uptimePort}. Process is now marked as running.`);
});
// ------------------------------------------
